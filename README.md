...
[Возможности](#features) · [Что мы сделали](#work) · [Архитектура](#architecture) · [Модель и результаты](#model) · [Демонстрация](#demo) · [Запуск](#quick-start) · [Разработчикам](#development) · [Проверки](#checks)
...
> Текущая телеметрия турбин ещё не подключена. Прогноз строится по актуальной погоде; последние измерения оборудования показываются отдельно как архивные, с датой.
...
**Индикатор надёжности** усредняет полноту погодных данных, `100 × (1 − MAE)` и свежесть выпуска. За отсутствие текущей телеметрии и независимого сравнения погодных источников снимается по 10 баллов; использование кэша снимает ещё 10. Нижняя граница итоговой оценки — 0.
...
Мощность, границы и измерения передаются в долях 0–1; округление выполняется при отображении. `observed: null` означает отсутствие текущего измерения. Последнее архивное значение возвращается отдельно в `lastObservation`.
...
- **Текущая телеметрия.** Последние исходные измерения относятся к 31 января 2026. Текущие `observed` и `observedAt` равны `null`, состояние оборудования — `unknown`; архивные значения подписаны датой.
...
- **Развёртывание.** Команды `dev` запускают локальные сервисы на loopback. Аутентификация и подключение SCADA пока не реализованы; для сетевого развёртывания потребуются HTTPS, обратный прокси и контроль доступа.
