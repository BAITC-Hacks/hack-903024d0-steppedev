...
export function ForecastExplanation({
  ...
  return (
    <Dialog
      ...
      description="Understand the conditions behind the expected output."
      drawer
    >
      {record && (
        <>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <span className="text-sm text-muted">
              {dateLabel(record.timestamp)} · {timeLabel(record.timestamp)} UTC
            </span>
          ...
          <Panel className="my-5 p-5">
            <p className="text-xs text-muted">Expected power</p>
            <p className="my-2 text-4xl font-medium">{percent(record[activeTurbine].prediction)}</p>
            <p className="text-xs text-muted">
              Expected range{' '}
              <span className="text-emerald-300">
                {percent(record[activeTurbine].lower)}–{percent(record[activeTurbine].upper)}
              </span>
            </p>
          </Panel>
          {error && (
          ...
          {explanation && (
            <>
                ...
                {explanation.drivers.map((driver) => (
                  <div key={driver.label}>
                    <div className="mb-2 flex justify-between text-xs">
                      ...
                      <span className="text-[10px] text-muted">{driver.note}</span>
                    </div>
              ...
              <p className="mt-4 flex items-center gap-1.5 text-[10px] text-muted">
                <Info size={11} />
                Illustrative driver values for this demo.
              </p>
              <div className="mt-8 border-t border-white/10 pt-6">
                ...
                <p className="mt-2 text-xs text-muted">
                  {explanation.historicalCount} historical periods · illustrative reference sample
                </p>
                ...
                >
                  {showPeriods ? 'Hide similar periods' : 'View similar periods'}
                  <ArrowUpRight size={14} />
                </Button>
                {showPeriods && (
                  <div className="mt-4 space-y-3 rounded-lg bg-white/[.025] p-4">
                    <p className="text-[10px] text-muted">EXAMPLE ARCHIVE MATCHES</p>
                    {explanation.periods.map((period) => (
                      <div key={period.timestamp} className="flex justify-between text-xs">
                        <span>
                          {dateLabel(period.timestamp)} · {timeLabel(period.timestamp)}
                        </span>
                        <span className="text-emerald-300">{percent(period.power)}</span>
                      </div>
                    ))}
                  </div>
                )}
